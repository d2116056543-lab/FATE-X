...
data_path = None
raw_video_dir = "/data/hdd01/jinbu/BDDX/BDD-V/Videos/videos/"
...
def process_video_chunk(item, image_size=56, num_frames=32):
    ...
    # for item in items:
    caption_id = item['id']
    vid_path = op.join(raw_video_dir, item['vidName'] + '.mov')
...
def main():
    ...
    output_folder = os.environ.get(
        "ADAPT_BDDX_FRAME_TSV_DIR",
        f"/data/jinbu/BDD-V/video_preprocess/frame_tsv")
    os.makedirs(output_folder, exist_ok=True)
    # To generate a tsv file:
    # data_path: path to raw video files
    global data_path, raw_video_dir

    image_size = int(os.environ.get("ADAPT_BDDX_IMAGE_SIZE", "256"))

    num_frames = int(os.environ.get("ADAPT_BDDX_NUM_FRAMES", "32"))

    # FIXME: the frame save dir generated by extract_frames.py
    data_path = os.environ.get(
        "ADAPT_BDDX_FRAME_DIR",
        f"/data/jinbu/BDD-V/data/{num_frames}frames/")
    raw_video_dir = os.environ.get(
    ...
    num_workers = int(os.environ.get("ADAPT_BDDX_NUM_WORKERS", "32"))

    # FIXME: the caption annotation file
    video_info_tsv = os.environ.get(
        "ADAPT_BDDX_CAPTIONS_JSON",
        '/data/jinbu/BDD-V/data/processed/captions_BDDX.json')
