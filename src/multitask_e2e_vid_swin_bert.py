...
class MultitaskVideoTransformer(torch.nn.Module):
    ...
    def __init__(self, args, config, swin, transformer_encoder):
        ...
        self.fate_x_control_reducer = getattr(args, 'fate_x_control_reducer', 'none')
        ...
        if self.fate_x_enabled and self.video_token_reducer != 'none':
            self.fate_x_reducer = VideoTokenReducer(
                ...
                temporal_tokens=getattr(args, 'fate_x_temporal_tokens', None),
                spatial_tokens_per_frame=getattr(args, 'fate_x_spatial_tokens_per_frame', None),
                min_tokens_per_frame=getattr(args, 'fate_x_min_tokens_per_frame', 1),
    ...
    def forward(self, *args, **kwargs):
        ...
        # By default only the caption/VL branch receives reduced/event tokens.
        # Control signal prediction keeps dense video tokens unless explicitly
        # requested via fate_x_reduce_control.
        if self.fate_x_enabled:
            vid_feats_text = self._apply_fate_x_tokens(vid_feats_dense, text_kwargs)

        text_kwargs['img_feats'] = vid_feats_text
        control_kwargs['img_feats'] = vid_feats_dense
        self.fate_x_last_stats.update({
            ...
            'control_visual_tokens': int(vid_feats_dense.shape[1]),
        ...
        if self.only_signal:
            ...
            if self.learn_mask_enabled:
                ...
                learn_att_full_len = self.max_img_seq_length
                vid_att_len = min(int(vid_feats_text.shape[1]), int(self.max_img_seq_length))
                learn_att = self.learn_vid_att.weight.reshape(learn_att_full_len, learn_att_full_len)[:vid_att_len, :vid_att_len]
