...
python -m torch.distributed.launch --nproc_per_node=1 --nnodes=1 --node_rank=0 --master_port="${MASTER_PORT:-45978}" src/tasks/run_adapt.py \
  ...
  --per_gpu_eval_batch_size "${PER_GPU_EVAL_BATCH_SIZE:-8}" \
  ...
  --loss_sensor_w 0.05 \
  --max_grad_norm 1 \
  --output_dir "${OUTPUT_DIR:-./output/repro_single_gpu/multitask_sensor_course_speed}" \
  "$@"
